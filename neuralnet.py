...
def random_init(shape):
    ...
    init_w = np.random.uniform(low = -0.1, high = 0.1, size = shape)
    return init_w
...
class SoftMaxCrossEntropy:
    ...
    def _cross_entropy(self, y: int, y_hat: np.ndarray) -> float:
        ...
        """
        # TODO: implement
        enc = np.zeros_like(y_hat) # creating one hot encoded vec
        enc[y] = 1
        # compute cross-entropy loss using log-sum-exp
        probs = np.log(y_hat)
        likelihood = probs.dot(enc)
        partition = np.logaddexp.reduce(probs)
        return partition - likelihood

    def forward(self, z: np.ndarray, y: int) -> Tuple[np.ndarray, float]: # not in
        """
        ...
        # raise NotImplementedError
        predictions = self._softmax(self, z)
        loss = self._cross_entropy(self, y, z)
...
class Sigmoid:
    def __init__(self):
        ...
        #  backward pass here
        # raise NotImplementedError
        self.values = {} # store results during forward prop and use during backward prop
    ...
    def forward(self, x: np.ndarray) -> np.ndarray:
        ...
        sig = 1 / (1 + np.exp(-x))
        self.values["sig"] = sig # avoid recomputing
        return sig
    ...
    def backward(self, dz: np.ndarray) -> np.ndarray:
        ...
        # TODO: implement
        # raise NotImplementedError
        d_sig = dz * self.values["sig"] * (1 - self.values["sig"])
        return d_sig
...
class Linear:
    ...
    def forward(self, x: np.ndarray) -> np.ndarray:
        ...
        #  the backward pass
        # raise NotImplementedError
        # add bias value of 1 to the beginning of the input array
        input_array = np.insert(x, 0, 1)
        # cache the weight matrix and input array with bias for backprop
        self.values["input_array"] = input_array

        # calculate the dot product of the weight matrix and input array with bias
        return np.dot(self.w, input_array)
    ...
    def backward(self, dz: np.ndarray) -> np.ndarray:
        ...
        # to calculate the partial derivative of the loss with respect to the input x (input_array),
        # we need to take the transpose of the weight matrix and multiply it by dz.
        dx = np.dot(self.w[:, 1:].T, dz)
        
        # to set the gradient of weights with respect to loss, we reshape dz and input_array, 
        # and then take their outer product.
        dz_reshaped = dz[:, np.newaxis]
        input_reshaped = self.values["input_array"][np.newaxis, :]
        self.dw = np.dot(dz_reshaped, input_reshaped)
...
class NN:
    ...
    def forward(self, x: np.ndarray, y: int) -> Tuple[np.ndarray, float]:
        ...
        # raise NotImplementedError
        first_lin_layer = self.linear1.forward(x)
        sig_activation = self.sig.forward(first_lin_layer)
        second_lin_layer = self.linear2.forward(sig_activation)
        y_hat = self.smce._softmax(second_lin_layer)
        loss = self.smce._cross_entropy(y, y_hat) 
        return (y_hat, loss)
    ...
    def backward(self, y: int, y_hat: np.ndarray) -> None:
        ...
        # raise NotImplementedError
        deriv = self.smce.backward(y, y_hat)
        second_lin_layer_grad = self.linear2.backward(deriv)
        sig_activ_grad = self.sig.backward(second_lin_layer_grad)
        first_lin_layer_grad = self.linear1.backward(sig_activ_grad)
        return None
    ...
    def compute_loss(self, X: np.ndarray, y: np.ndarray) -> float:
        ...
        num_points = 0
        for x, y, in zip(X, y):
            y_hat_and_loss = nn.forward(x, y)
            loss += self.smce._cross_entropy(y, y_hat_and_loss[0])
            num_points += 1
    ...
    def train(self, X_tr: np.ndarray, y_tr: np.ndarray,
        ...
        for epoch in range(n_epochs):
            ...
            # train the network using the shuffled data
            for x, y in zip(X_tr_shuffled, y_tr_shuffled):
                y_hat, loss = nn.forward(x, y)
                nn.backward(y, y_hat) # just want y_hat from the returned tuple
                nn.step()

            # evaluate the network on training and test data
           
            train_loss = nn.compute_loss(X_tr, y_tr)
            test_loss = nn.compute_loss(X_test, y_test)
    ...
    def test(self, X: np.ndarray, y: np.ndarray) -> Tuple[np.ndarray, float]:
        ...
        # raise NotImplementedError
        error = 0
        predicted_labels = np.empty_like(y)
        for i in range(len(X)):
            y_hat, loss = nn.forward(X[i], y)
            predicted_labels[i] = np.argmax(y_hat)
            if y[i] != predicted_labels[i]: # check if truth label != pred label
                error += 1
...
if __name__ == "__main__":
    ...
    for n in [5, 20, 50, 100, 200]:
        D[n] = train_losses[-1]
        DPrime[n] = test_losses[-1]
    train_vals = list(D.values())
    valid_vals = list(DPrime.values())
    plt.plot(np.arange(5), train_vals, label = "train cross entropy loss")
    plt.plot(np.arange(5), valid_vals, label = "val cross entropy loss")
    plt.xlabel("Hidden Units")
    plt.ylabel("Avg Training Cross Entropy")
    plt.legend()
    plt.show()
    ...
    # plt.plot(n_hid, plot_train_losses, label='train')
    # plt.plot(n_hid, plot_val_losses, label='val')
    # plt.xlabel('Number of hidden units')
    # plt.ylabel('Average cross-entropy loss')
    # plt.legend()
    # plt.show()
    # plt.plot(list(range(n_hid)), train_losses)
    # plt.plot(list(range(n_hid)), test_losses)
